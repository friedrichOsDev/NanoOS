...
void hpet_init() {
    ...
    serial_printf(COM1, "HPET: tick period: %u fs, ticks/us: %llu, ticks/ms: %llu\n", tick_period, hpet_ticks_per_us, hpet_ticks_per_ms);
    ...
    if (hpet_regs->timers[0].configuration_and_capability & (1 << 4)) {
        ...
        serial_printf(COM1, "HPET: warning Timer 0 does NOT support periodic mode!\n");
    }
...
void hpet_udelay(uint64_t microseconds) {
    if (!hpet_regs) return;
    uint64_t target_ticks = hpet_regs->main_counter_value + (microseconds * hpet_ticks_per_us);
...
void hpet_mdelay(uint64_t milliseconds) {
    if (!hpet_regs) return;
    uint64_t target_ticks = hpet_regs->main_counter_value + (milliseconds * hpet_ticks_per_ms);
