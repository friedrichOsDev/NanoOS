...
void hpet_init() {
    if (!hpet) {
        panic("FATAL: System requires HPET, but none was found in ACPI tables.",
              0);
    }
    ...
    serial_printf(
        COM1, "HPET: tick period: %u fs, frequency: %llu Hz, ticks/ms: %llu\n",
        tick_period_fs, hpet_frequency_hz, hpet_ticks_per_ms);

    // 2. Stop counter, reset to 0
    hpet_regs->general_configuration &=
        ~3ULL; // Disable counter + legacy routing
    ...
    for (uint32_t i = 0; i < num_timers; i++) {
        hpet_regs->timers[i].configuration_and_capability &=
            ~(1ULL << 2); // Disable IRQ
    }
    ...
    serial_printf(COM1,
                  "HPET: free-running counter started (%d timers disabled)\n",
                  num_timers);
}
