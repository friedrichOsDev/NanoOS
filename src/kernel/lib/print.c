...
static int print_formatted(void* dest, size_t size, const void* format, va_list args, bool is_wide) {
    ...
    #define PUSH_CHAR(c) do { \
        if (dest && i < size - 1) { \
            if (is_wide) ((uint32_t*)dest)[i] = (uint32_t)(c); \
            else ((char*)dest)[i] = (char)(c); \
        } \
        i++; \
    } while(0)
    ...
    while ((c = GET_CHAR()) != 0) {
        ...
        switch (c) {
            ...
            case 'i': {
                int64_t val_d;
                if (length_mod == 2 || length_mod == 1) val_d = va_arg(args, long);
                else if (length_mod == 3) val_d = va_arg(args, size_t);
                ...
                uint32_t w_buf[64];
                char a_buf[64];
                int len = is_wide ? uint_to_str(abs_val, w_buf, 10) : uint_to_str_legacy(abs_val, a_buf, 10);
                int pad = (width > len) ? (width - len) : 0;

                if (!r_width && !pad_zero) while (pad-- > 0) PUSH_CHAR(' ');
                if (!r_width && pad_zero) while (pad-- > 0) PUSH_CHAR('0');

                for (int j = 0; j < len; j++) PUSH_CHAR(is_wide ? w_buf[j] : (uint32_t)(unsigned char)a_buf[j]);
                if (r_width) while (pad-- > 0) PUSH_CHAR(' ');
                break;
            }
            ...
            case 'o': {
                uint64_t val_u;
                if (length_mod == 2 || length_mod == 1) val_u = va_arg(args, unsigned long);
                ...
                uint32_t w_buf[64];
                char a_buf[64];
                int len = is_wide ? uint_to_str(val_u, w_buf, base) : uint_to_str_legacy(val_u, a_buf, base);
                ...
                if (c == 'x' || c == 'X') { PUSH_CHAR('0'); PUSH_CHAR(c); }
                else if (c == 'o') { PUSH_CHAR('0'); PUSH_CHAR('o'); }
            ...
            case 's': {
                ...
                if (is_wide) {
                    ...
                    if (!r_width) while (pad-- > 0) PUSH_CHAR(' ');
                    for (int j = 0; j < len; j++) PUSH_CHAR(s[j]);
                } else {
            ...
            case 'p': {
                ...
                uint32_t w_buf[64];
                char a_buf[64];
                int len = is_wide ? uint_to_str(val_p, w_buf, 16) : uint_to_str_legacy(val_p, a_buf, 16);
                int pad = (width > len + 2) ? (width - (len + 2)) : 0;

                if (!r_width && !pad_zero) while (pad-- > 0) PUSH_CHAR(' ');
                PUSH_CHAR('0'); PUSH_CHAR('x');
                if (!r_width && pad_zero) while (pad-- > 0) PUSH_CHAR('0');
    ...
    if (dest && size > 0) {
        ...
        if (is_wide) ((uint32_t*)dest)[last] = 0;
        else ((char*)dest)[last] = '\0';
    }
